...
def run_algorithm(Solution_no, Max_iter, F_name, max_runs=10):
    ...
    for run in range(max_runs):
        Best_F, _, curve = RSA_PSO_Hybrid(Solution_no, Max_iter, LB, UB, Dim, F_obj)
    ...
    return fitness_mean, fitness_std, avg_curve
...
def main():
    ...
    for i in range(13, 14):
        F_name = f"F{i}"
        print(f"\n▶ Running {F_name} ({max_runs} runs)...")
        mean_fit, std_fit, avg_curve = run_algorithm(Solution_no, Max_iter, F_name, max_runs)
        print(f"✔ Done: Mean = {mean_fit:.6e}, Std = {std_fit:.6e}")
        results.append([F_name, mean_fit, std_fit])

        # Plot convergence
        plt.plot(avg_curve, label=F_name)

    plt.title("Average Convergence Curve (RSA-PSO)")
